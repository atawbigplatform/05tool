...
export module ui {
    ...
    export class TreeNodeReact extends domFile.Core.DomReact<TreeNodeProps, TreeStates, TreeAction> implements domFile.Core.IReact {
        ...
        private h_Send(s: string, className?: string): React.ReactElement<any> {
            ...
            var _i = <span><i className={!this.props.Vm.NoExpand ? this.props.Vm.ExpandIcon : this.props.Vm.NoExpandIcon} onClick={(e) => { e.stopPropagation(); this.ExpandClick(); }}></i><i className="fa fa-folder"></i></span>;
            var _ii = <i className={(this.props.Vm.NodeIcoSrc)}></i>
           // var _icon = _isLeaf ? _ii : _ii;
            var _icon = _isLeaf ? _ii : _ii;
            var _iicon = _isLeaf ? "" : _i;
            var _ul = <ul className={"nav nav-1 " + (!this.props.Vm.NoExpand ? "Ha-tree-animation" : "hide")}>
                {
            ...
            if (!this.props.Vm.TreeObj.NodeTplFun) {
                return <li className={className} key={this.props.Vm.key}>
                    <a className={_c + " "} onClick={(e) => { this.ActiveNode(); e.stopPropagation() }}> 
                        {_isEast ?
                                [_icon, <span>{_codeIcon}< span className="Hf-text-overflow"   title={this.props.Vm.Text} dangerouslySetInnerHTML={{ __html: this.props.Vm.TreeObj.isSearch ? this.Search_HighLight(this.props.Vm.Text) : this.props.Vm.Text }}></span></span>, this.props.children]
                                : [_iicon, <span>{_codeIcon} < span className="Hf-text-overflow" title={this.props.Vm.Text} dangerouslySetInnerHTML={{ __html: this.props.Vm.TreeObj.isSearch ? this.Search_HighLight(this.props.Vm.Text) : this.props.Vm.Text }}></span></span>, this.props.children]
                ...
                return <li className={className} key={this.props.Vm.key}>
                    <a className={_c} onClick={(e) => { this.ActiveNode(); e.stopPropagation() }}>
                        {_isEast ?
                                [_icon, <span>{_codeIcon}< span className="Hf-text-overflow" title={this.props.Vm.Text}> {_tpl}</span></span>, this.props.children]
                                : [_iicon, <span>{_codeIcon} < span className="Hf-text-overflow"  title={this.props.Vm.Text}> {_tpl}</span></span>, this.props.children]
                        }
                    </a>
                    {_nodeTpl}
                    <div>{this.props.Vm.Mesg}</div>
        ...
        private Search_HighLight(Text: string) {
    ...
    export class TreeNodeTplReact extends domFile.Core.DomReact<TreeNodeTplProps, any, any> implements domFile.Core.IReact {
        ...
        public pSender(): React.ReactElement<any> {
            ...
            return <span>{this.props.Vm.TreeNode.Text}</span>
        }
    ...
    export class TreeVm extends domFile.Core.DomVm implements domFile.Core.IVm {
     ...
     //   @decorator.setDecoratorProps("小标题", "", "树形菜单")
        public Subtext: string;
     //   @decorator.setDecoratorProps("何时使用", "", "◇ ")
        public WUse: string;
     //   @decorator.setDecoratorProps("如何使用", "", "1.引入组件  import Tree = require(\"./../../05tool/05tool/ Tree\");")
        public HUse: string;
      ...
      //  @decorator.setDecoratorProps("高度","number","0，表示跟页面可视高度一样")
        public Height: number = 0;//树的高度，0表示跟页面可视高度一样
      ...
      //  @decorator.setDecoratorProps("样式类型","string","H,")
        ...
        public isSearch: boolean = false;//是否是在搜索树
        public HighLightKey: string = "";//搜索时候高亮的关键字
        ...
        public copyNode(nodeVM: TreeNodeVm): TreeNodeVm {
    ...
    export class TreeNodeVm extends domFile.Core.DomVm implements domFile.Core.IVm {
        ...
        public NoExpandIcon: string = "icon-plus-sign  fa fa-caret-right";
        public ExpandIcon: string ="icon-minus-sign  fa fa-caret-down";
        public IsActive: boolean;
