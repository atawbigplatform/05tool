...
import  * as utilFile from "01core/Util";
import  * as iocFile from "01core/Ioc";
import  * as urlFile  from "01core/Url";
import  * as decoratorFile from "01core/Decorator";
...
export module BreadDom {
    ...
    export class BreadDomReact extends domCore.DomReact<BreadDomProps, BreadDomStates, BreadDomAction> implements domCore.IReact {
        ...
        private _senderUL(): React.ReactElement<any> {
            if (this.props.Vm.NextLinkList && this.props.Vm.NextLinkList.length > 0) {
                return <ul className={"nav ACT-BREAD-UL Hz-scroll clearfix" + (this.props.Vm.IsExpand ? "" : " hide") }  style={{ overflow: "auto" }}>
                    {
        ...
        public pSender(): React.ReactElement<any> {
            ...
            return <div className="Hu-breadcrumb YSu-breadcrumb">
                ...
                return item.intoDom(i);
                    ...
                    <li>
                        <a className={((this.props.Vm.NextLinkList && this.props.Vm.NextLinkList.length > 0) ? "" : "hide")} onClick={(e) => { this._clickEapandFun(); e.stopPropagation(); }}>
                            <i className={"ACT-BREAD-ICON " + (this.props.Vm.IsExpand ? " Ha-transform  " : " ") + ("icon-caret-right fa fa-caret-right ") }></i>
                        </a>
                        {this._senderUL() }
                    </li>
  ...
  //  @decorator.setDecoratorWUse("hk")
    ...
    export class BreadDomVm extends domCore.DomVm implements IReactBreadDomVm {
        ...
        protected pRegName = "面包屑";
        // public BreadName: string = "面包名称";

    //    @decorator.setDecoratorProps("小标题","显示当前页面在系统层级结构中的位置，点击按钮能显示菜单导航")
        public Subtext: string;
     //   @decorator.setDecoratorProps("何时使用", "◇ 当需要告知用户『你在哪里』时")
        public WUse: string;
     ...
     //   @decorator.setDecoratorProps("是否展开","boolean")
